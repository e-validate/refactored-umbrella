...
join chatrooms
on chatrooms.chatroom_id = user_favorite_chatrooms.chatroom_id
...
-- where user_id = 44 and chatroom_id in ( select chatroom_id from matches 
-- group by chatroom_id having count(*)>1)
-- )
-- where m.chatroom_id in (select chatroom_id from user_favorite_chatrooms where user_id = 39);
